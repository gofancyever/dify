...
export const webAppLogin: Fetcher<LoginResponse, {
 url: string;
body: Record<string, any>
}> = ({ url, body }) => {
...
export const sendWebAppForgotPasswordEmail: Fetcher<CommonResponse & { data: string }, {
 url: string;
body: { email: string }
}> = ({ url, body }) =>
  post<CommonResponse & { data: string }>(url, { body }, { isPublicAPI: true })

export const verifyWebAppForgotPasswordToken: Fetcher<CommonResponse & {
 is_valid: boolean;
email: string
}, {
 url: string;
body: { token: string }
  ...
  return post(url, { body }, { isPublicAPI: true }) as Promise<CommonResponse & {
 is_valid: boolean;
email: string
}>
}

export const changeWebAppPasswordWithToken: Fetcher<CommonResponse, {
 url: string;
body: {
 token: string;
new_password: string;
...
token: string
 ...
 is_valid: boolean;
token: string
}>('/forgot-password/validity', { body }, { isPublicAPI: true })
...
export const verifyShufengToken = (body: { sf_token: string }) =>
  post<CommonResponse & {
 access_token: string;
refresh_token: string
}>('/oauth/shufeng/token', { body })
